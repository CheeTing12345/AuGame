...
export default function TowerGame() {
  ...
  const handleDownload = () => {
    const myName      = me?.getState('name')      || 'You'
    const partnerName = partner?.getState('name') || 'Partner'
    const date        = new Date().toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })

    const lines = []
    lines.push('Common Ground — Session Results')
    lines.push(`${date}`)
    lines.push('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━')
    lines.push('')
    lines.push('OVERALL STATS')
    lines.push(`  Best run:    ${bestRun} block${bestRun !== 1 ? 's' : ''}`)
    lines.push(`  Avg sync:    ${avgAlignment}%`)
    lines.push(`  Collapses:   ${totalCollapsed}`)
    lines.push('')
    lines.push('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━')
    lines.push('')
    lines.push('ANSWERS BY QUESTION')
    lines.push('')
    ...
    if (questionIds) {
      questionIds.forEach((qid, i) => {
        ...
        if (!q) return
        const mA = me?.getState(`answer_${i}`)      ?? '—'
        const pA = partner?.getState(`answer_${i}`) ?? '—'
        const sync = (mA !== '—' && pA !== '—')
          ? Math.round((1 - Math.abs(mA - pA) / 4) * 100)
          : '—'

        lines.push(`Q${String(i + 1).padStart(2, '0')}  ${q.prompt}`)
        lines.push(`      Scale: 1 = "${q.leftLabel}"  →  5 = "${q.rightLabel}"`)
        lines.push(`      ${myName.padEnd(12)} ${mA}/5`)
        lines.push(`      ${partnerName.padEnd(12)} ${pA}/5`)
    ...
    lines.push('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━')
    lines.push('Generated by Common Ground')

    const blob = new Blob([lines.join('\n')], { type: 'text/plain' })
    const url  = URL.createObjectURL(blob)
    const a    = document.createElement('a')
    a.href     = url
    a.download = `common-ground-${date.replace(/ /g, '-')}.txt`
    a.click()
